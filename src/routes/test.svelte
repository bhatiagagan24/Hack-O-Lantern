	...
	async function generateOtp() {
		if (!!email) {
			...
			const otp_resp = await fetch(url)
				...
				.then((data) => (otp_recieved = data.toString()));
...
{email} -->
		...
		<div class="flex flex-col items-center align-middle justify-center text-center">
			<h1 style="color: white;" class="text-xl">Enter Your Email</h1>
			<input type="email" class="email" bind:value={email} />
			<button class="bg-green-300 hover:bg-red-600 text-white font-bold py-2 px-4 rounded" on:click={verifyTheOtp}>Generate OTP</button>
		...
		{#if otp_recieved != 1}
			...
			<div class="flex flex-col align-middle justify-center text-box-div">
				<div
					class="py-2 grid gap-2 grid-cols-6 md:grid-cols-6 align-middle justify-center text-center"
				>
				...
				<span class="flex flex-col text-center align-middle justify-center">
					<input
						type="button"
						on:click={verifyTheOtp}
						name="Verify"
						value="Verify"
						class="align-middle justify-center text-center w-1/2 md:w-1/2"
...
{#if wrong_otp === true}
	...
	<div class="text-xl text-center">
		<img
			src="https://media.giphy.com/avatars/christianwhite/eLlCBMGqniDh.gif"
			style="height:50vh; width:30%; margin-left: 35%; margin-right: 35%;"
		/>
...
<style>
	...
	main {
		...
		/* bottom: 0; */
		position: fixed;
		background-image: url('https://i.gifer.com/77g.gif');
